...
const DEFAULT_EAP_APPS_SCRIPT_URL = 'https://script.google.com/macros/s/AKfycbx4hAEe5i_ulWGSl9qfiokoCGzMza3QzUDIlM4cuZV_8eRw-Ml3XltdAbD0K0EFWm9x4Q/exec';
...
function getCryptoKey() {
  return String(import.meta.env.VITE_JSON_CRYPTO_KEY || '').trim();
}
...
async function decryptEnvelope<T>(envelope: EncryptedJsonEnvelope): Promise<T> {
  ...
  for (let i = 0; i < cipherBytes.length; i += 1) {
    if (offset >= block.length) {
      block = await sha256Bytes(`${cryptoKey}|${envelope.nonce}|${counter}`);
      counter += 1;
      offset = 0;
    }
  ...
  const requestPromise = (async () => {
    ...
    for (const url of getPublicJsonUrls(fileName)) {
      try {
        ...
        if (isEncryptedEnvelope(payload)) return decryptEnvelope<T>(payload);
        return payload as T;
      } catch (error) {
...
export async function fetchEapAppsScriptData<T>() {
  const url = String(import.meta.env.VITE_EAP_APPS_SCRIPT_URL || DEFAULT_EAP_APPS_SCRIPT_URL).trim();
  const response = await fetch(url, { cache: 'no-store' });
  const payload = await response.json();

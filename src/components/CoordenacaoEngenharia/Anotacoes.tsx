...
import { FileSpreadsheet, FileText, Globe, Lock, MoreVertical, Plus, Trash2, X } from 'lucide-react';
import { getDisciplineDisplayName } from '../Atividades';
import { exportNoteToCsv, exportNoteToPdf } from '../../lib/noteExport';
...
type ContextMenuState = { row: number; col: number; x: number; y: number } | null;
...
export default function Anotacoes({
  filter, sheets, osOptions, disciplinaOptions, currentUser, templates, onSave, onDelete, onSaveTemplate, onDeleteTemplate, controlledSheet, onCloseControlled,
}: AnotacoesProps) {
  const [editing, setEditing] = React.useState<AnnotationSheet | null>(() => controlledSheet ?? null);
  ...
  React.useEffect(() => {
    if (controlledSheet) setEditing(controlledSheet);
  }, [controlledSheet]);
  ...
  const filteredSheets = sheets.filter((sheet) => {
    const matchesFilter = filter.type === 'all'
      ? true
      : filter.type === 'disciplina' ? sheet.disciplina === filter.value : sheet.osCodigo === filter.value;
    if (!matchesFilter) return false;
    const isPublica = sheet.publica !== false;
    return isPublica || sheet.autorEmail === currentUser.email;
  });
  const meusTemplates = templates.filter((tpl) => tpl.autorEmail === currentUser.email);
  ...
  if (creating) {
    const criarSheet = (base: { titulo: string; colCount: number; rows: string[][] }) => {
      setEditing({
        id: makeId('note'),
        disciplina: filter.type === 'disciplina' ? filter.value : '',
    ...
    return (
      <div className="rounded-xl border border-[#E5E7EB] bg-[#F9FAFB] p-5">
        <p className="mb-3 text-[13px] font-bold text-[#2D2D2D]">Quantas colunas a planilha vai ter?</p>
        <div className="flex items-center gap-3">
          ...
          <button
            type="button"
            onClick={() => criarSheet({ titulo: '', colCount: newColCount, rows: createEmptyRows(newColCount, 4) })}
        ...
        {meusTemplates.length > 0 && (
          <div className="mt-4 border-t border-[#E5E7EB] pt-4">
            <p className="mb-2 text-[12px] font-bold text-[#2D2D2D]">Ou comece a partir de um template seu:</p>
            <div className="flex flex-wrap gap-2">
              {meusTemplates.map((tpl) => (
                <div key={tpl.id} className="flex items-center overflow-hidden rounded-lg border border-[#E5E7EB] bg-white hover:border-[#F7C7B7]">
                  <button
                    type="button"
                    onClick={() => criarSheet({ titulo: tpl.titulo, colCount: tpl.colCount, rows: tpl.rows.map((row) => [...row]) })}
                    className="px-3 py-2 text-[12px] font-bold text-[#2D2D2D] hover:text-[#F05D28]"
                  >
                    {tpl.nome}
                  </button>
                  <button
  ...
  if (editing) {
    ...
    const updateCell = (r: number, c: number, value: string) => setEditing((prev) => {
      if (!prev) return prev;
      const rows = prev.rows.map((row, ri) => (ri === r ? row.map((cell, ci) => (ci === c ? value : cell)) : row));
      return { ...prev, rows };
    });
    const insertRow = (at: number) => setEditing((prev) => {
      if (!prev) return prev;
      const rows = [...prev.rows];
      rows.splice(at, 0, Array.from({ length: prev.colCount }, () => ''));
      return { ...prev, rows };
    });
    const removeRow = (at: number) => setEditing((prev) => {
      if (!prev || prev.rows.length <= 1) return prev;
      return { ...prev, rows: prev.rows.filter((_, ri) => ri !== at) };
    });
    const insertCol = (at: number) => setEditing((prev) => {
      if (!prev) return prev;
      ...
      return { ...prev, rows, colCount: prev.colCount + 1 };
    });
    const removeCol = (at: number) => setEditing((prev) => {
      if (!prev || prev.colCount <= 1) return prev;
      return { ...prev, rows: prev.rows.map((row) => row.filter((_, ci) => ci !== at)), colCount: prev.colCount - 1 };
    });
    ...
    const updateTexto = (texto: string) => setEditing((prev) => (prev ? { ...prev, texto } : prev));
    ...
    const disciplinaPendente = !editing.disciplina.trim();
    ...
    const handleSaveTemplate = async () => {
      ...
      await onSaveTemplate({
        ...
        colCount: editing.colCount,
        rows: editing.rows.map((row) => [...row]),
        autorEmail: currentUser.email,
        criadoEm: new Date().toISOString(),
      });
    ...
    const menuActions: Array<[string, () => void]> = contextMenu ? [
      ['Inserir linha acima', () => insertRow(contextMenu.row)],
      ['Inserir linha abaixo', () => insertRow(contextMenu.row + 1)],
      ['Remover linha', () => removeRow(contextMenu.row)],
      ['Inserir coluna à esquerda', () => insertCol(contextMenu.col)],
      ['Inserir coluna à direita', () => insertCol(contextMenu.col + 1)],
      ['Remover coluna', () => removeCol(contextMenu.col)],
    ] : [];
      ...
      <div className="fixed inset-0 z-[200] flex flex-col overflow-hidden bg-white">
        <div className="flex items-center justify-between gap-4 border-b border-slate-200 px-5 py-2.5">
          <div className="flex min-w-0 items-center gap-3">
            <h2 className="truncate text-[15px] font-black text-[#1F2937]">{editing.titulo || 'Nova anotação'}</h2>
            {autorInfo && <span className="whitespace-nowrap text-[11px] text-slate-400">{autorInfo}</span>}
          ...
          <div className="flex items-center gap-2 flex-shrink-0">
            <button
              type="button"
              onClick={() => setTemplateNamePrompt(editing.titulo || 'Novo template')}
        ...
        <div className="min-h-0 flex-1 overflow-auto p-5">
          ...
          <div className="mt-3 overflow-auto rounded-xl border border-[#E5E7EB]">
                    ...
                    {row.map((cell, c) => (
                      <td
                        ...
                        onContextMenu={(event) => {
                          ...
                          setContextMenu({ row: r, col: c, x: event.clientX, y: event.clientY });
                        }}
          ...
          <div className="mt-4 rounded-xl border border-[#E5E7EB] p-4">
            <h4 className="mb-2 text-[13px] font-bold text-[#2D2D2D]">Notas</h4>
            <textarea
              value={editing.texto || ''}
              onChange={(event) => updateTexto(event.target.value)}
              placeholder="Escreva livremente aqui, como um bloco de texto..."
              rows={5}
              ...
              <div className="mt-2 flex flex-wrap gap-2">
                {linkedNotes.map((note) => (
                  <span key={note.id} className="inline-flex items-center gap-1.5 rounded-full border border-[#E5E7EB] bg-[#F9FAFB] py-1 pl-3 pr-1.5 text-[12px] font-medium text-[#2D2D2D]">
                    <button type="button" onClick={() => setEditing(note)} className="hover:text-[#F05D28]">
  ...
  const renderCard = (sheet: AnnotationSheet) => {
    ...
    const subtitulo = filter.type === 'os'
      ? getDisciplineDisplayName(sheet.disciplina)
      : (os ? `OS ${os.codigo} - ${os.nome}` : (filter.type === 'all' ? getDisciplineDisplayName(sheet.disciplina) : 'Personalizado'));
    const autorData = [sheet.autorNome, formatDateBR(sheet.criadoEm)].filter(Boolean).join(' · ');
    const isPublica = sheet.publica !== false;

    return (
      <div key={sheet.id} className="relative rounded-xl border border-[#E5E7EB] bg-white p-4 transition-colors hover:border-[#F7C7B7]">
        <div
          role="button"
          tabIndex={0}
          onClick={() => setEditing(sheet)}
          onKeyDown={(event) => { if (event.key === 'Enter') setEditing(sheet); }}
        ...
        <button
          ...
          className="absolute right-2 top-2 flex h-6 w-6 items-center justify-center rounded-full text-[#94A3B8] hover:bg-[#F3F4F6] hover:text-[#2D2D2D]"
        >

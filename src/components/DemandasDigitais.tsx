...
import { Bug, CheckCircle2, ClipboardList, Clock3, FileSpreadsheet, GripVertical, Lightbulb, MoreHorizontal, Pencil, Trash2 } from 'lucide-react';
...
interface DemandasDigitaisProps {
  ...
  onEdit?: (id: string, values: { title: string; body: string }) => Promise<void> | void;
}
...
export default function DemandasDigitais({ reports, noteDemands = [], canAccess, loading, error, onStatusChange, onArchive, onEdit }: DemandasDigitaisProps) {
...
{loading ? <div className="rounded-3xl border border-[#E5E7EB] bg-white p-10 text-center text-[13px] font-medium text-[#757575]">Carregando demandas...</div> : error ? <div role="alert" className="rounded-3xl border border-red-200 bg-red-50 p-6 text-center text-[13px] font-medium text-red-700">{error}</div> : <div className="grid min-h-[360px] grid-cols-1 gap-4 md:grid-cols-2 xl:grid-cols-4">{columns.map((column) => <KanbanColumn key={column.key} column={column} reports={reports.filter((report) => report.status !== 'archived' && displayStatus(report.status) === column.key)} demands={noteDemands.filter((demand) => demand.status === column.label)} onStatusChange={onStatusChange} onArchive={onArchive} onEdit={onEdit} />)}</div>}
  </section>;
}
...
function KanbanColumn({ column, reports, demands, onStatusChange, onArchive, onEdit }: { key?: React.Key; column: typeof columns[number]; reports: DemandFeedbackReport[]; demands: NoteDemand[]; onStatusChange?: DemandasDigitaisProps['onStatusChange']; onArchive?: DemandasDigitaisProps['onArchive']; onEdit?: DemandasDigitaisProps['onEdit'] }) {
  return <div className="rounded-2xl border border-[#E5E7EB] bg-white/70 p-3"><div className="mb-3 flex items-start justify-between gap-2 px-1"><div><h2 className="text-[13px] font-black text-[#2D2D2D]">{column.label}</h2><p className="mt-1 text-[11px] text-[#94A3B8]">{column.hint}</p></div><span className="rounded-full bg-[#F9FAFB] px-2 py-1 text-[11px] font-black text-[#757575]">{reports.length + demands.length}</span></div><div className="space-y-3">{reports.map((report) => <DemandCard key={report.id} report={report} onStatusChange={onStatusChange} onArchive={onArchive} onEdit={onEdit} />)}{demands.map((demand) => <NoteDemandCard key={demand.id} demand={demand} />)}</div>{reports.length === 0 && demands.length === 0 && <div className="rounded-xl border border-dashed border-[#E5E7EB] px-3 py-8 text-center text-[11px] font-medium text-[#A0A7B2]">Nenhum pedido aqui</div>}</div>;
}

function DemandMenu({ title, onArchive, onEdit }: { title: string; onArchive?: () => void; onEdit?: () => void }) {
  const [open, setOpen] = React.useState(false);
  if (!onArchive && !onEdit) return null;
  return <div className="relative"><button type="button" aria-label={`Ações: ${title}`} title="Ações do card" onClick={(event) => { event.stopPropagation(); setOpen((value) => !value); }} className="flex h-8 w-8 items-center justify-center rounded-lg text-[#64748B] hover:bg-[#F1F5F9] focus:outline-none focus:ring-2 focus:ring-[#F05D28]"><MoreHorizontal size={17} /></button>{open && <div className="absolute right-0 top-9 z-10 min-w-28 rounded-lg border border-[#E5E7EB] bg-white p-1 shadow-lg">{onEdit && <button type="button" onClick={(event) => { event.stopPropagation(); setOpen(false); onEdit(); }} className="flex w-full items-center gap-2 rounded-md px-3 py-2 text-left text-[12px] font-bold text-[#374151] hover:bg-[#F9FAFB]"><Pencil size={14} />Editar</button>}{onArchive && <button type="button" onClick={(event) => { event.stopPropagation(); setOpen(false); if (window.confirm(`Arquivar “${title}”? O registro não será apagado.`)) onArchive(); }} className="flex w-full items-center gap-2 rounded-md px-3 py-2 text-left text-[12px] font-bold text-[#DC2626] hover:bg-[#FEF2F2]"><Trash2 size={14} />Excluir</button>}</div>}</div>;
}

function DemandCard({ report, onStatusChange, onArchive, onEdit }: { key?: React.Key; report: DemandFeedbackReport; onStatusChange?: DemandasDigitaisProps['onStatusChange']; onArchive?: DemandasDigitaisProps['onArchive']; onEdit?: DemandasDigitaisProps['onEdit'] }) {
